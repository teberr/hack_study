...
암호문은 일단 기본적으로 mod 연산이기 때문에 암호문 C와 내가 원하는 값r을 암호화 한 식을 곱한걸 mod N을 통해 다시 크기를 작게 해줘도 결과가 같지 않을까? 라는 생각을 했다. 이게 잘 되는지 고민하던 가장 큰 이유는 임의의 암호문과 FLAG_enc를 곱해준 결과값이 다음 식이 성립하는지가 고민이었다. (결과값을 d승만큼 하고 나머지 연산을 해야하는데 미리 결과값을 d승하기 전에 나머지 연산을 해버리면 이 d승값에도 영향이 가서 결과가 같을지?가 고민이었음)

![고민거리.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/2d8a6bb7-b4b6-400d-b5b0-6c77965894e0/고민거리.png)

[https://ko.khanacademy.org/computing/computer-science/cryptography/modarithmetic/a/modular-exponentiation](https://ko.khanacademy.org/computing/computer-science/cryptography/modarithmetic/a/modular-exponentiation)

그런데 여기서 모듈러 성질중에 

![칸아카데미.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/94340eea-1c6c-407e-8cd1-813c3cbfc74b/칸아카데미.png)

이러한 성질이 있다는 것을 보고 아 결과값이 너무 크게나오면 미리 N으로 모듈러 연산을 해줘도 되겠구나!를 깨달았다.
그래서 새롭게 다시 연결하여 해봤다.
