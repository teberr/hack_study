https://honey-push-30b.notion.site/Carve-party-577a1d9f6ec041f189426026d8f81c87 에서 사진 깨짐 없이 볼 수 있다.

# Carve party

## 문제 파일 다운로드

![문제 파일 다운로드.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/a2c3a482-4a9b-4020-9bd6-a3df4d0dd9aa/문제_파일_다운로드.png)

## 문제 파일

![문제파일.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/6af09da2-668e-414d-80aa-ac19c6402ff2/문제파일.png)

다운로드 받은 압축파일을 풀면 jack-o-lantern.html 문제파일이 나온다.

## Jack-o-lantern

![jack-o-lantern.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/53559efb-3d76-4f4a-be56-bb30b8c4663d/jack-o-lantern.png)

jack-o-lantern 페이지를 열면 호박이 나온다. 이 호박을 10000번 누르라고 되어있다. F12를 눌러서 개발자 도구를 켜 source를 확인하면 아래와 같은 부분이 나온다. 
...
이제 이 코드 중 호박을 클릭했을 때 실행되는 부분인 make함수 부분과 $('#jack-target').click 함수를 자세히 살펴보면 된다. 
...
make 함수를 위에서 부터 살펴보면 counter 의 값에 따라서 jack의 코, 왼쪽,오른쪽, 입에 css 효과를 주는 것을 확인할 수 있다. 그리고 클릭했을 때 10000에서 counter만큼 뺀 값을 텍스트로 보여주는 것을 확인할 수 있다.

![click.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/2129c074-c5a0-41d2-9474-f668def09fd5/click.png)

실제로 클릭 두번을 하면 아래 숫자가 10000에서 9998로 줄어든 것을 볼 수 있다.
...
호박을 클릭할 때마다 counter값은 1씩 증가하고 이 counter 값이 10000보다 작고 100의 배수일 때 pumpkin 배열에 있는 값들을 xor연산하고 pie값도 xor연산 및 and 연산으로 pie값을 바꿔준다. 즉 이 함수를 10000번 넘게 실행하면 click을 10000번 넘게 한 것과 같은 효과를 주므로 이 함수를 10000번 넘게 실행해본다.

## 개발자 도구 console을 통해 함수 실행

```jsx
for(var i=0;i<=10000;i++){$('#jack-target').click()}
```

를 개발자 도구 console에서 실행하여 자바스크립트를 실행할 수 있다.

![console.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/f8bdebc2-2654-47aa-9a4a-3756ab33c0ca/console.png)

실행하고 나면 페이지에 flag값이 나온다.

![flag.PNG](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/d8c72446-befb-484b-9b20-ac8006c955e9/flag.png)

flag값은 DH{I_lik3_pumpk1n_pi3}로 나온다.
